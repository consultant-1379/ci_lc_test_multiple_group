...
/**
 * Audits the Statistical subscriptions to ensure that all expected scanners are present in DPS
 * For Statistical system defined subscription, This auditor will not correct missing/duplicate node. It will just call Task Validator.
 */
@Singleton
@Lock(LockType.READ)
public class StatisticalSubscriptionAuditor implements SubscriptionAuditor {
    ...
    public List<Long> getActiveSubscriptionIds() {
        return resourceSubscriptionHelper.getActiveSubscriptionIds(SubscriptionType.STATISTICAL);
    }
}
