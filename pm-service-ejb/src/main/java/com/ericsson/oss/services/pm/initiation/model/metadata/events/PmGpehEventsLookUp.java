...
/**
 * The type Pm events look up for gpeh events.
 */
public class PmGpehEventsLookUp extends AbstractPmEventsLookUp {
    ...
    protected Collection<EventTableRow> fetchEventsAndGroups(final ModelInfo modelInfo) {
        final Collection<EventTableRow> result =
                getProfileEvents(modelInfo, PmMetaDataConstants.GPEH_PROFILE_PATTERN, EventTypeFilter.TRIGGERED);
        result.addAll(getProfileEvents(modelInfo, PmMetaDataConstants.GPEH_PROFILE_PATTERN, EventTypeFilter.NON_TRIGGERED));
        return result;
    }
