...
import com.ericsson.oss.services.pm.initiation.task.factories.deactivation.RpmoSubscriptionDeactivationTaskRequestFactory
import com.ericsson.oss.services.pm.initiation.tasks.SubscriptionDeactivationTaskRequest

class RpmoSubscriptionDeactivationTaskRequestFactorySpec extends PmServiceEjbSkeletonSpec {

    @ObjectUnderTest
    private RpmoSubscriptionDeactivationTaskRequestFactory rpmoSubscriptionDeactivationTaskRequestFactory
    ...
    def "Will create tasks for all nodes if each node has USERDEF scanner with subscription name but will not track"() {
        ...
        def subMo = dps.subscription().type(SubscriptionType.RPMO).name("Test").administrationState(AdministrationState.DEACTIVATING).nodes(nodeMO1, nodeMO2).build()
        scannerUtil.builder("USERDEF-Test.RPMO", "GSM02BSC01").node(nodeMO1).subscriptionId(subMo).status(ScannerStatus.ACTIVE).build()
        scannerUtil.builder("USERDEF-Test.RPMO", "GSM02BSC02").node(nodeMO2).subscriptionId(subMo).status(ScannerStatus.ACTIVE).build()

        def nodes = nodeDao.findAll()
        def sub = subscriptionDao.findOneById(subMo.getPoId()) as RpmoSubscription
        when:
        def tasks = rpmoSubscriptionDeactivationTaskRequestFactory.createMediationTaskRequests(nodes, sub, false)
        then:
        tasks.size() == 2
        tasks.each {
    ...
    def "Will create tasks for all nodes if each node has USERDEF scanner with subscription name regardless of state of scanner"() {
        ...
        def nodeMO2 = nodeUtil.builder("GSM02BSC02").neType(NetworkElementType.BSC).build()
        def subMo = dps.subscription().type(SubscriptionType.RPMO).name("Test").administrationState(AdministrationState.DEACTIVATING).nodes(nodeMO1, nodeMO2).build()
        scannerUtil.builder("USERDEF-Test.RPMO", "GSM02BSC01").node(nodeMO1).subscriptionId(subMo).status(ScannerStatus.INACTIVE).build()
        scannerUtil.builder("USERDEF-Test.RPMO", "GSM02BSC02").node(nodeMO2).subscriptionId(subMo).status(ScannerStatus.ERROR).build()

        def nodes = nodeDao.findAll()
        def sub = subscriptionDao.findOneById(subMo.getPoId()) as RpmoSubscription
        when:
        def tasks = rpmoSubscriptionDeactivationTaskRequestFactory.createMediationTaskRequests(nodes, sub, true)
        then:
        tasks.size() == 2
        tasks.each {
            assert it.getJobId().contains("subscriptionId=" + sub.getId())
    ...
    def "will not create any deactivation tasks if nodes do not exist"() {
        given:
        def subMo = dps.subscription().type(SubscriptionType.RPMO).name("Test").administrationState(AdministrationState.DEACTIVATING).build()

        def sub = subscriptionDao.findOneById(subMo.getPoId()) as RpmoSubscription
        when:
        def tasks = rpmoSubscriptionDeactivationTaskRequestFactory.createMediationTaskRequests([], sub, true)
        then:
        tasks.size() == 0
