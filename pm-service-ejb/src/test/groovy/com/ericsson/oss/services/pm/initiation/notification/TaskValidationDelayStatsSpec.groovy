...
class TaskValidationDelayStatsSpec extends PmServiceEjbSkeletonSpec {
    ...
    def "The subscription goes to ERROR after timeout if there are no scanners"() {
        ...
        subscriptionMO = statisticalSubscriptionBuilder.name("Test").taskStatus(TaskStatus.OK).administrativeState(AdministrationState.ACTIVE).build()
    ...
    def "The subscription goes from ERROR to OK after timeout if there are two nodes and two scanners"() {
        ...
        Mockito.when(timer.currentTimeMillis()).thenReturn(System.currentTimeMillis())
        subscriptionMO = statisticalSubscriptionBuilder.name("Test").taskStatus(TaskStatus.ERROR).administrativeState(AdministrationState.ACTIVE).build()
        dpsUtils.addAssociation(subscriptionMO, "nodes", nodes.get(0), nodes.get(1))
        createScanner()
    ...
    def "The subscription goes from NA to OK after timeout if there are two nodes and two scanners"() {
        given: "Two nodes and two scanners"
        Mockito.when(timer.currentTimeMillis()).thenReturn(System.currentTimeMillis())
        subscriptionMO = statisticalSubscriptionBuilder.name("Test").taskStatus(TaskStatus.NA).administrativeState(AdministrationState.ACTIVE).build()
        dpsUtils.addAssociation(subscriptionMO, "nodes", nodes.get(0), nodes.get(1))
        createScanner()
        createScanner(NODE_NAME_2)
        delayedTaskStatusValidator.scheduleDelayedTaskStatusValidation(subscriptionMO.poId)
    ...
    def "The subscription is deactivated if there are no nodes"() {
    ...
    def "All subscription eligible for audit should be validated and task status should be updated"() {
        given: "Two nodes and two scanners"
        Mockito.when(timer.currentTimeMillis()).thenReturn(System.currentTimeMillis())
        subscriptionMO = statisticalSubscriptionBuilder.name("Test").taskStatus(TaskStatus.NA).administrativeState(AdministrationState.ACTIVE).build()
        dpsUtils.addAssociation(subscriptionMO, "nodes", nodes.get(0), nodes.get(1))
        createScanner()
        createScanner(NODE_NAME_2)
        and: "schedule audit for invalid Subscription "
        delayedTaskStatusValidator.scheduleDelayedTaskStatusValidation(12346L)
        and: "schedule audit for valid Subscription "
        delayedTaskStatusValidator.scheduleDelayedTaskStatusValidation(subscriptionMO.poId)
        when: "The validation timer times out"
        Mockito.when(timer.currentTimeMillis()).thenReturn(System.currentTimeMillis() + 30000)
        delayedTaskStatusValidator.validateTaskStatusAdminState()
    ...
    private createScanner(final String nodeName = NODE_NAME_1) {
        scannerUtil.builder("USERDEF.TEST.Cont.Y.Stats", nodeName)
