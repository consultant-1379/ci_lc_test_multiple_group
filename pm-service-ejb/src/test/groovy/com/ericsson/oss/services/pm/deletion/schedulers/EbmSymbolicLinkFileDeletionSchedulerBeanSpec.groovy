...
import static com.ericsson.oss.services.pm.initiation.utils.TestConstants.EBM_FILE_COLLECTION_DIR
...
class EbmSymbolicLinkFileDeletionSchedulerBeanSpec extends AbstractFileDeletionSchedulerBeanSpec {
    ...
    @ObjectUnderTest
    EbmSymlinkDeletionSchedulerBean deletionScheduler
    ...
    def "When delete symlinks is called should delete symlinks"() {
        when:
        fileSystemHelper.createTestFiles(EBM_FILE_COLLECTION_DIR, DAYS_5 * HOURS_24 * FILES_PER_HR, MINUTES_PER_ROP)
        fileSystemHelper.createTestSymlinks(EBM_FILE_COLLECTION_DIR, SYMLINK_DIRECTORY, MINUTES_PER_ROP)
    ...
    def cleanup() {
        fileSystemHelper.deleteTestFiles(EBM_FILE_COLLECTION_DIR)
        fileSystemHelper.deleteTestDirectories(EBM_FILE_COLLECTION_DIR)
