...
public class UetrSubscriptionITTest extends InputBaseArquillian {

    Logger logger = LoggerFactory.getLogger(UetrSubscriptionITTest.class);
    ...
    @InSequence(4)
    @OperateOnDeployment(PM_SERVICE_TEST)
    public void createUetrSubscriptionTest() throws InterruptedException {
        testResponseHolder.clear();
        final CountDownLatch cl1 = new CountDownLatch(1);
        testResponseHolder.setCountDownLatch(cl1);
        final SubscriptionOperationRequest createMessage = new SubscriptionOperationRequest(PmServiceRequestsTypes.CREATE, null, null,
        ...
        for (final SubscriptionInfo subInfo : subscriptionList) {
            logger.debug("Created uetr subscription with name : {}, adminState : {}, taskStatus : {}", subInfo.getName(),
    ...
    public void cleanUpSubscription() throws InterruptedException {
        final SubscriptionOperationRequest deleteMessage = new SubscriptionOperationRequest(PmServiceRequestsTypes.DELETE, null, null,
                getClass().getSimpleName());
