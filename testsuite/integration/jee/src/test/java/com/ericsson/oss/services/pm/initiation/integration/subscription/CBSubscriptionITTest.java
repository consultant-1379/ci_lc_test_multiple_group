...
public class CBSubscriptionITTest extends InputBaseArquillian {

    Logger logger = LoggerFactory.getLogger(CBSubscriptionITTest.class);
    ...
    @InSequence(4)
    @OperateOnDeployment(PM_SERVICE_TEST)
    public void createCBSubscription() throws InterruptedException {
        ...
        for (final SubscriptionInfo subInfo : subscriptionList) {
            logger.debug("Created Criteria Base Subscription with name : {}, cbsStatus : {}", subInfo.getName(), subInfo.getCbs());
            Assert.assertEquals(true, subInfo.getCbs());
